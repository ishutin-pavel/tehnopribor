	...
	static function constructYML($name){
		...
		if (strlen($res) > 1) {
			...
			while ($row = DB::fetchAssoc($res)) {
				...
				if ($row['parent'] > 0 && (
					($options['catsType'] == 'fromCats' && in_array($row['parent'], $catsForExport)) || 
					($options['catsType'] == 'selected' && in_array($row['parent'], $options['catsSelect'])) ||
					$options['catsType'] == 'all')
				) {
			...
			while ($row = DB::fetchAssoc($res)) {
				...
				if ($printMain) {
					...
					$xml->writeElement('url', SITE.'/'.$product['category_url'].'/'.$product['url'].$utm);
