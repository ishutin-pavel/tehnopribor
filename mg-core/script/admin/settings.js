  ...
  return {
    ...
    */
    init: function() {
      ...
      $('.admin-center').on('click', '.section-settings .openLanding', function() {        
      ...
      $('.admin-center').on('change', '.section-settings  select[name="landingName"]', function() {
        settings.drawColorShemesLanding($(this).find('option:selected').data('schemes'));       
      });

      $('.section-settings  select[name="landingName"]').trigger('change');
      ...
      // Выбор цветовой схемы шаблона
      $('.admin-center').on('click', '.section-settings .color-scheme-landing', function() { 
      ...
      $('body').on('change', '#addLanding', function() {
        settings.addNewTemplate('newLandingForm');
      });
    ...
    */
    ...
    getAllSetting: function(tab) {
      ...
      if(tab == "tab-template-settings" ) {
        ...
        obj.colorSchemeLanding = $('.color-scheme-landing.active').data('scheme');
        if (obj.colorSchemeLanding == undefined) {
          obj.colorSchemeLanding = 'none';
        }
      }     
    ...
    openDeliveryModalWindow: function(type, id) {
      ...
      switch (type) {
        case 'edit': {          
          ...
          settings.loadLocaleDelivery(id);
          break;
        }
        case 'add': {
    ...
    updataTabs: function() {
      ...
      function(response) {
        ...
        $.each(response.data.deliveryArray, function(i, delivery) {
          ...
          $('tr#delivery_'+delivery.id+' td#paymentHideMethod').text(paymentMethod).data('weight', delivery.weight);
