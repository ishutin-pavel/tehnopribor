...
class Property {
		  	...
		  	if(($value['type'] == 'string')||($value['type'] == 'textarea')) {
	  			...
	  			$table = 'product_user_property_data';
	  		  	...
	  		  	while ($userFieldsData = DB::fetchAssoc($res)) {
	  		  		...
	  		  		if(empty($userFieldsData['margin'])) $userFieldsData['margin'] = $userFieldsData['margin_orig'];
		    		$data[] = $userFieldsData;
	  		  	}
	  		  	$table = 'property_data';
		  	}
	...
	public static function getEasyPropToCsv($id, $color, $size) {
		...
		while ($row = DB::fetchAssoc($res)) {
			...
			if(empty($row['name'])) {
				$names[$_SESSION['export']['propColumns'][$row['id']]] = $row['name_orig'].$color;
			} else {
				$names[$_SESSION['export']['propColumns'][$row['id']]] = $row['name'].$color;
			}
		}
