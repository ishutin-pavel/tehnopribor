  ...
  return {
    ...
    init: function() {
      ...
      $('.admin-center').on('click', '.section-catalog .add-new-button', function() {
        ...
        $('.landingLink').attr('href', 'javascript:void(0);');
        $('.landingLink').text('');
      ...
      $('body').on('click', '.section-catalog .set-size-map', function() {
        $('.size-map').slideToggle();
      });

      // клик для автогенерации мета тегов при открытии таба с метой
      $('body').on('click', '.section-catalog .auto-meta', function() {
        catalog.generateSeoFromTmpl();
      });
      ...
      $('body').on('change', '#add-product-wrapper .storageToView', function() {
      ...
      $('.admin-center').on('click', '.section-catalog .edit-row', function() {
        ...
        var link = $(this).closest('tr').find('.fa-external-link').attr('href') + '?lp';
        $('#lndAcc').show();
        $('.landingLink').attr('href', link);
        $('.landingLink').text(link);
        ...
        function(response) {
          ...
          if (response.status == 'success') {
            ...
            if (mainurl.indexOf('no-img.') < 0) {
              ...
              var row = catalog.drawControlImage(src, true,'','',response.data);
            ...
            if(response.data.length) {
              ...
              if (act == 'add' && imgCount > 0) {
                for (var i = 0; i < imgCount; i++) {
                  ...
                  var row = catalog.drawControlImage(src, true,'','',response.data[i]);
                  $('.sub-images').append(row);
                  // if ($('#add-product-wrapper input[name="title"]').val().length) {
                  //   $('.images-block img:last').attr('alt', $('#add-product-wrapper input[name="title"]').val());
                  // }
     ...
     // Обработчик для загрузки изображения на сервер, сразу после выбора.
      ...
      $('body').on('change', 'select[name=landingIndividualTemplate]', function() {
      ...
      $('body').on('click', '#add-product-wrapper .color-scheme-landing', function() { 
        $(this).parents('ul').find('.color-scheme-landing').removeClass('active');
        $(this).addClass('active');
      });

      // Сброс картинки лендинга
      $('body').on('click', '#add-product-wrapper .remove-added-background', function() { 
        $('.landing-img-block img').attr('src', admin.SITE+'/uploads/no-img.jpg');
      ...
      $('body').on('change', '#add-product-wrapper .upload-form-landing input[name="landingBackground"]', function() {
        var currentImg = '';
      ...
      if(!main) {
        ...
        return '<div class="image-item parent" data-filename="'+filename+'">\
                  ...
                  <div class="img-action-hover">\
                    <div class="elem">\
                      <a class="fa fa-check top set-main-image btn" href="javascript:void(0);" title="'+lang.VALUE_DEFAULT+'" aria-hidden="true"></a>\
                    </div>\
                    <div class="elem">\
                      <a class="fa fa-cogs top seo-image btn" aria-hidden="true btn" href="javascript:void(0);" title="'+lang.SEO_SET+'"></a>\
                    </div>\
                    <div class="elem">\
                      <form class="imageform" method="post" noengine="true" enctype="multipart/form-data">\
                        <label class="btn tip icon" title="'+lang.UPLOAD_IMG+'">\
                    ...
                    <div class="elem">\
                      <a class="btn tip icon fl-right cancel-img-upload" href="javascript:void(0);" title="'+lang.DELETE_IMAGE+'">\
      ...
      return function(response) { 
        ...
        $('.variant-row').each(function() {   
          ...
          if($(this).find('input[name=color]').val() != '') {
            color = '[data-color='+$(this).find('input[name=color]').val()+']';
          } else {
            color = '';
          }
          if($(this).find('input[name=size]').val() != '') {
            size = '[data-size='+$(this).find('input[name=size]').val()+']';
          } else {
            size = '';
          }
          $('.size-map .checkbox input'+color+size).prop('checked', true);
        });
    ...
    */
    ...
    cteateTableVariant:function(variants, imageDir) {
      ...
      if(variants) {
        ...
        <tr class="text-left">\
          <th></th>\
          <th class="hide-content" style="width:150px;position:relative;">'+lang.NAME_VARIANT+'\
