  ...
  public static function multiLangLink($data = '') {
    if(@LANG != 'LANG' && @LANG != 'default') {
      ...
      foreach ($matches[1] as &$item) {
        if((substr_count($item, SITE) == 1)&&(substr_count($item, '.png') != 1)&&(substr_count($item, '.jpg') != 1)) {
          $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $item);
        }
      }
      $data = strtr($data, $newLink);

      preg_match_all('/<form[^>]*action=\"?([^>]*>)/', $data, $matches);
      unset($matches[0]);
      $newLink = array();
      foreach ($matches[1] as &$item) {
        if((substr_count($item, SITE) == 1)&&(substr_count($item, SITE.'/'.LANG) < 1)) {
          $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $item);
        }
        if(substr_count($item, '/'.LANG) < 1) {
          $site = explode('/', SITE.$item);
          $site = implode('/', array_unique($site));
          $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $site);
        }
      }
      $data = strtr($data, $newLink);
    }
