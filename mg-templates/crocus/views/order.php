...
<h1 class="j-title">Оформление заказа</h1>
<div class="checkout-form-wrapper">
    ...
    <div class="payment-option">
        <form class="j-order__form" action="<?php echo SITE ?>/order?creation=1" method="post">

            <div class="j-order__form__left">
                <div class="j-order__form__title">
                    Форма заказа
                </div>
                <div class="form-list">
                    <input type="text" name="email" value="<?php echo $_POST['email'] ?>" placeholder="Email*">
                    <input type="text" name="phone" value="<?php echo $_POST['phone'] ?>" placeholder="Телефон*">
                    <input type="text" name="fio" value="<?php echo $_POST['fio'] ?>"     placeholder="Ф.И.О.">
                    <textarea class="address-area" name="address"                         placeholder="Адрес доставки"><?php echo $_POST['address'] ?></textarea>
                    <textarea class="address-area" name="info"                            placeholder="Комментарий к заказу"><?php echo $_POST['info'] ?></textarea>

                    <div class="j-order__form__title   delivery-text">Плательщик:</div>
                ...
                <div class="form-list yur-field" <?php echo $style ?>>
                    <input type="text" name="yur_info[nameyur]" value="<?php echo $_POST['yur_info']['nameyur'] ?>" placeholder="Юр. лицо">
                    <input type="text" name="yur_info[adress]" value="<?php echo $_POST['yur_info']['adress'] ?>"   placeholder="Юр. адрес">
                    <input type="text" name="yur_info[inn]" value="<?php echo $_POST['yur_info']['inn'] ?>"         placeholder="ИНН">
                    <input type="text" name="yur_info[kpp]" value="<?php echo $_POST['yur_info']['kpp'] ?>"         placeholder="КПП">
                    <input type="text" name="yur_info[bank]" value="<?php echo $_POST['yur_info']['bank'] ?>"       placeholder="Банк">
                    <input type="text" name="yur_info[bik]" value="<?php echo $_POST['yur_info']['bik'] ?>"         placeholder="БИК">
                    <input type="text" name="yur_info[ks]" value="<?php echo $_POST['yur_info']['ks'] ?>"           placeholder="К/Сч">
                    <input type="text" name="yur_info[rs]" value="<?php echo $_POST['yur_info']['rs'] ?>"           placeholder="Р/Сч">
                </div>
            </div>

            <div class="j-order__form__right">
                <div class="delivery-vs-payment">
                    <?php if(''!=$data['delivery']): ?>
                    <div class="j-order__form__title delivery-text">
                        Доставка
                    </div>
                    <ul class="j-order__form__ul   delivery-details-list">
                    ...
                    <?php endif; ?>
                    <div class="j-order__form__delivery-date   delivery-date" style="display:none;">
                    ...
                    <ul class="payment-details-list">
                        ...
                        <li class="j-alert j-alert__default">Укажите способ доставки, чтобы выбрать способ оплаты</li>
                        <?php elseif(''!=$data['paymentArray']): ?>
                        <?php echo $data['paymentArray'] ?>
                        <?php else: ?>
                        <li class="j-alert j-alert__red">Нет доступных способов оплаты</li>
                        <?php endif; ?>
                    </ul>
                </div>
            </div>

            <div class="j-order__form__bottom">
                <div class="j-order__form__total   summ-info">
                    К оплате:
...
break; case 2: $data['meta_title'] = 'Оплата заказа'; mgSEO($data); if($data['msg']): ?>
    <div class="j-alert j-alert__red errorSend"> <?php echo $data['msg'] ?> </div>
<?php endif; ?>

<h1 class="j-title">Оплата заказа</h1>
<?php if(!$data['pay']&&$data['payment']=='fail'): ?>
    <div class="j-alert j-alert__red payment-form-block"> <?php echo $data['message']; ?> </div>
...
break; case 3: $data['meta_title'] = 'Подтверждение заказа'; mgSEO($data); if($data['msg']): ?>
    <div class="j-alert j-alert__green text-success"> <?php echo $data['msg'] ?> </div>
<?php endif; if($data['id']): ?>

<h1 class="j-title">Подтверждение заказа</h1>
<p class="j-alert j-alert__green auth-text">Заказ №<?php echo $data['orderNumber'] ?> подтвержден</p>

<?php endif; if($data['active']): ?>
    <div class="j-alert j-alert__green text-success">
        Вы успешно зарегистрировались на сайте <strong><?php echo SITE ?></strong> и можете отслеживать заказ в личном кабинете.
    </div>
    <div class="j-alert j-alert__default get-login text-center">
...
break; case 4: ?>
    <h1 class="j-title">Оплатите заказ № <?php echo $data['orderNumber'] ?> на сумму <?php echo MG::numberFormat($data['summ']) ?> <?php echo $data['currency'] ?></h1>
    <?php if($data['payMentView']){ include($data['payMentView']); } else{ ?>
        <div class="j-alert j-alert__red">Ваш способ не предусматривает оплату электронными деньгами <br> Вы должны оплатить заказ в соответствии с указанным способом оплаты!</div>
    <?php }



break; case 5: ?>
    <h1 class="j-title">Статус заказа</h1>
    <?php if ($data['msg']) { ?>
        <div class="j-alert j-alert__red errorSend"> <?php echo $data['msg']; ?> </div>
