...
class Controllers_Order extends BaseController {
    ...
    if (isset($_POST['toOrder'])) {
      ...
      if ($error = $model->isValidData($_POST)) {
		...
		if (URL::get('addOrderOk')) {	
		  ...
		  $_SESSION['post'] = $_POST;
		  MG::redirect('/order?addOrderOk=1');
		}	
  ...
  public function getPaymentView($pay) {
    ...
    if (file_exists($dir.'mg-templates'.DIRECTORY_SEPARATOR.MG::getSetting('templateName').DIRECTORY_SEPARATOR.'layout'.DIRECTORY_SEPARATOR.'payment_'.$payMentView)) {
      $payMentView2 = $dir.'mg-templates'.DIRECTORY_SEPARATOR.MG::getSetting('templateName').DIRECTORY_SEPARATOR.'layout'.DIRECTORY_SEPARATOR.'payment_'.$payMentView;
    }
    elseif (file_exists($dir.'mg-core'.DIRECTORY_SEPARATOR.'layout'.DIRECTORY_SEPARATOR.'payment_'.$payMentView)) {
      $payMentView2 = $dir.'mg-core'.DIRECTORY_SEPARATOR.'layout'.DIRECTORY_SEPARATOR.'payment_'.$payMentView;
    }
    else{
