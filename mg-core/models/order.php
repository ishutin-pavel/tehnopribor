...
class Models_Order {
  ...
  public function isValidData($arrayData, $require = array('email','phone','payment'),$createUser = true , $error = null) {
    ...
    if (!empty($error)) {
      ...
      // делаем массив полей опциональных
      foreach ($optionalFields as $item) {
        $fields[] = MG::translitIt($item['name']);
      }
      // кастомные поля заносим
      ...
      $this->storage = $arrayData['storage'];
  ...
  public function addOrder($adminOrder = false) {
    ...
    // $this->optFields
    foreach ($this->optFields as $key => $value) {
    ...
    if ($id) {
      ...
      MG::storageMinusProduct($this->storage);
      ...
      foreach ($this->optFields as $key => $val) {
        foreach ($opF as $val2) {
          if(MG::translitIt($val2['name']) == $key) {
      ...
      if ($order[$orderId]['status_id'] == 4) {
        ...
        foreach ($order_content as $item) {
          if(!MG::enabledStorage()) {
            ...
            MG::decreaseCountProductOnStorage($item ,$order[$orderId]['storage']);
          }
        }
      }
    }    
  ...
  public function cloneOrder($id) {
    ...
    foreach ($content as $item) {
      if(MG::enabledStorage()) {
        MG::decreaseCountProductOnStorage($item, $storage);
      } else {
        $product->decreaseCountProduct($item['id'], $item['code'], $item['count']);
      }
    }
    ...
    foreach ($codes as $prod) {
      if ($prod['count'] > 0) {
        if(!MG::enabledStorage())
          ...
          MG::increaseCountProductOnStorage($prod, $order[$orderId]['storage']);
      } elseif ($prod['count'] < 0) {
        if(!MG::enabledStorage())
          $product->decreaseCountProduct($prod['id'], $prod['code'], abs($prod['count']));
        else
          MG::decreaseCountProductOnStorage($prod, $order[$orderId]['storage']);   
      }
    }
   ...
   * <code>
   * $data = array('id' => 15);
   * $html = Models_Order::createCustomFieldToAdmin($data);
   * echo $html;
   * </code>
   * @param array $data параметры
   * @return string
   */
  public function createCustomFieldToAdmin($data) {
    $fields = unserialize(stripcslashes(MG::getSetting('optionalFields')));

    $res = DB::query('SELECT * FROM '.PREFIX.'custom_order_fields WHERE id_order = '.DB::quote($data['id']));
    while($row = DB::fetchAssoc($res)) {
      $rows[] = $row;
    }
    ...
    foreach ($fields as $item) {
      ...
      $data = null;
      foreach ($rows as $row) {
        if($row['field'] == MG::translitIt($item['name'])) {
          $data = $row;
        }
      }
      $editableField = '';
        switch ($item['type']) {
        case 'input':
          $editableField = '<input name="'.MG::translitIt($item['name']).'" class="order-edit-display customField" type="text" style="display:none1;" value="'.$data['value'].'">';
          break;
        case 'textarea':
          $editableField = '<textarea name="'.MG::translitIt($item['name']).'" class="order-edit-display customField" type="text" style="display:none1;">'.$data['value'].'</textarea>';
          break;
        case 'checkbox':
          $checbox = true;
          if(empty($data['value'])) {
          ...
          $editableField = '<div class="checkbox">
                            <input type="checkbox" id="custom-'.MG::translitIt($item['name']).'" name="'.MG::translitIt($item['name']).'" '.$checked.' class="customField">
                            <label for="custom-'.MG::translitIt($item['name']).'"></label>
                          </div>';
          break;
        case 'select':
        case 'radiobutton':
          $editableField = '<select name="'.MG::translitIt($item['name']).'" class="order-edit-display customField">
            <option></option>';
          foreach ($item['variants'] as $variant) {
            ...
            $editableField .= '<option value="'.$variant.'" '.$selected.'>'.$variant.'</option>';
          }
      ...
      if($checbox) {
        ...
        $inFields = '<label class="order-edit-visible">'.$data['value'].'</label>
                  '.$editableField;
      }
      

      $toHtml .= '<div class="row"><div class="large-4 medium-5 columns">
    ...
    if($rows) {
      $html = '<div style="border-top: 1px solid #e6e6e6;'.$hide.'"></div>
                <div class="row" style="padding: 5px 28px;">
                  <div class="medium-6 columns">
                    <div class="row">
                      <p><b>'.$lang['ADDITIONAL_INFO'].'</b></p>
                      '.$toHtml.'
                    </div>
                  </div>
                </div>';
