...
class Category {
  ...
  public function __construct() {
    ...
    $this->categories = Storage::get(md5('category'.@LANG));
    ...
    if($this->categories == null) {  
      ...
      if(MG::getSetting('catalogPreCalcProduct') == 'old') {
        ...
        if(MG::getSetting('printProdNullRem') == "true") {
          if(MG::enabledStorage()) {
            $onlyInCount = 'AND (SELECT SUM(ABS(count)) FROM '.PREFIX.'product_on_storage WHERE product_id = p.id)';
          } else {
      ...
      // }
      Storage::save(md5('category'.@LANG),$this->categories);
    }
 ...
 /**
  ...
  private static function resizeCategoryImg($file) {
    ...
    if ('svg' == strtolower($ext) || 'gif' == strtolower($ext)) {
      $name .= '.'.$ext;
      @copy($realDocumentRoot.$file, $realDocumentRoot.'/uploads/'.$name);
      return '/uploads/'.$name;
    }
...
//    unset($array['id']);
  ...
  public function updateCategory($array) {
    ...
    $filter = array('title','meta_title','meta_keywords','meta_desc','html_content','html_content-seo','unit','seo_title','seo_alt');
    $localeData = MG::prepareLangData($array, $filter, $lang);
    
    $maskField = array('title','meta_title','meta_keywords','meta_desc','image_title','image_alt','seo_title','seo_alt');
  ...
  public function delCategory($id) {
   ...
   public function getCategoriesHTML() {  
      $result = Storage::get(md5('getCategoriesHTML'.LANG));     
      ...
      if($result == null) {        
        ...
        // if ( @$_SESSION['user']->enabledSiteEditor == "false") {
          Storage::save(md5('getCategoriesHTML'.LANG),$result);        
        // }
      }
   ...
   public function getCategoriesHorHTML() {  
      $result = Storage::get(md5('getCategoriesHorHTML'.LANG));     
      ...
      if($result == null) {        
        ...
        // if ( $_SESSION['user']->enabledSiteEditor == "false") {
          Storage::save(md5('getCategoriesHorHTML'.LANG),$result);        
        // }
      }
  ...
  public function getHierarchyCategory($parent = 0, $onlyActive=false) {
    if($parent == 0 && !URL::isSection('mg-admin') && !$onlyActive) $result = Storage::get('getHierarchyCategory'.$parent.LANG);
    if($result == null) {
      $catArray = array();
      if (!empty($this->categories))
        foreach ($this->categories as $category) {       
          if(!isset($category['id'])) {break;}//если категории неceotcndetn
            ...
            if ($parent==$category['parent']) {
              $child = $this->getHierarchyCategory($category['id'], $onlyActive);
              ...
              if (!empty($child)) {
                ...
                if(MG::getSetting('catalogPreCalcProduct') == 'old') {
                  foreach($child as $item) {
                    if (array_key_exists('insideProduct', $item)) {
                      $array['insideProduct'] += $item['insideProduct'];
                    }
                  }
                  if (array_key_exists('countProduct', $category)) {
      ...
      $args = func_get_args();
      $result = $catArray;
      if($parent == 0 && !URL::isSection('mg-admin') && !$onlyActive) Storage::save('getHierarchyCategory'.$parent.LANG, $result);
    }
    return MG::createHook(__CLASS__."_".__FUNCTION__, $result, @$args);
  }
