<?php
	...
	if(file_exists(CORE_DIR.'updateError.txt')) {
		if((time() - filemtime(CORE_DIR.'updateError.txt')) > (60 * 1)) {
	...
	if($lastPhpVersion != $phpVersion) {
		...
		if (!$connection) {
		  	throw new Exception('Невозможно установить соединение.');
		}
		// проверяем подключение к базе
		if (!mysqli_select_db($connection,$config['DB']['NAME_BD'])) {
		  throw new Exception('Ошибка! Невозможно выбрать указанную базу.');
		}
		// достаем ключ
		$sql = 'SELECT `value` FROM '.$config['DB']['TABLE_PREFIX'].'setting WHERE `option` = \'licenceKey\'';
		$res = mysqli_query($connection,$sql) or die  ("Ошибка выполнения запроса:".mysqli_error($connection)."<br/>".$sql);
		...
		if($key) {
			...
			if(!$dontWork) {
				...
				if(file_exists($file)) {
					...
					echo $mes = 'Архив не найден! Произошла ошибка при скачивании архива обновления. Вероятно ваша версия PHP не поддерживается.<br>
						Поддерживаемые версии PHP 5.3, 5.4, 5.5, 5.6, 7.0, 7.1<br>У вас установлена версия PHP '.PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION.'<br>
						Если ваша версия удовлетворяет требованиям, то обратитесь в тех. поддержку Могуты';
