  ...
  return {
    ...
    */
    generateKeywords: function(title) {
      if (!$('#add-category-modal .seo-wrapper input[name=meta_keywords]').val()) {	 
        var keywords = title;
        var keyarr = title.split(' ');
        
        if(keyarr.length == 1) {
          $('#add-category-modal .seo-wrapper input[name=meta_keywords]').val(keywords);
          return;
        }
    ...
    */         
    ...
    runOperation: function(operation) { 
      ...
      if (!confirm(lang.RUN_CONFIRM)) {return false;}
      var dropProducts = 'false';
      if ((operation == 'delete' || operation == 'fulldelete') && confirm(lang.SUB_CATEGORY_DELETE_PROD)) {dropProducts = 'true';}
      admin.ajaxRequest({
