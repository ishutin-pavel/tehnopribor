...
class VKUpload {
	...
	*/
	static function getNum($shopCats, $inactiveToo){
		...
		$result = array('productIDs' => $productIDs, 'productsCount' => count($productIDs));
		return $result;
	}
   /**
	...
	*/
	static function upload($access_token, $vkCat, $vkAlbum, $productIDs, $useNull){
		...
		while (count($productIDs) > 0) {
			...
			$timerSave = microtime(true);
			if($timeHave < 0) {
				$result = array('productIDs' => $productIDs, 'remaining' => count($productIDs), 'errors' => $errors);
				return $result;
			}
	...
	* @return array массив с ID товаров магазина и количеством найденых товаров
	*/
	static function getNumDelete($shopCats){
	...
	*/
	static function delete($access_token, $productIDs){
