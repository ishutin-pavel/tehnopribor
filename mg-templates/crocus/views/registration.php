...
<h1 class="j-title">Регистрация пользователя</h1>

<?php if($data['message']): ?>
    <span class="j-alert j-alert__green"><?php echo $data['message'] ?></span>
<?php endif; ?>

<?php if($data['error']): ?>
    <span class="j-alert j-alert__red"><?php echo $data['error'] ?></span>
<?php endif; ?>
...
<div class="create-user-account-form">
    ...
    <form action="<?php echo SITE ?>/registration" method="POST">
        ...
        <button type = "submit" name="registration" class="register-btn j-button">Зарегистрироваться</button>
    </form>
    <div class="clear">&nbsp;</div>
</div>
<?php endif ?>