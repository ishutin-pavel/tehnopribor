  ...
  public function addImage($productImg = false, $watermark = false, $addPath = '') {
    ...
    if (isset($_POST) && 'POST' == $_SERVER['REQUEST_METHOD']) {
      ...
      if (strlen($name)) {
        ...
        if (strpos($ext2, 'svg') || strtolower($ext) == 'svg') {
          $isSvg = true;
        }
        else{
          $isSvg = false;
        ...
        if (in_array(strtolower($ext2), $validFormats) || $isSvg) {
          if (($size < (1024 * 5 * 1024) && !empty($file_array['tmp_name'])) || $isSvg) { //$file_array['tmp_name'] будет пустым если размер загруженного файла превышает размер установленный параметром upload_max_filesize в php.ini
            $name = rawurldecode($name);
            $name = str_replace(array(" ", "%"), array("-", ""), $name);    
            $name = MG::translitIt($name);
            $actualImageName = $this->prepareName($name, $ext);

            if ($watermark && empty($_FILES['landingBackground']) && !$isSvg) {
            ...
            if (move_uploaded_file($tmp, $path.$actualImageName) || copy($tmp, $path.$actualImageName)) {
              ...
              if (MG::getSetting("waterMark") == "true" && !$watermark && $productImg && empty($_FILES['landingBackground']) && !$isSvg) {
